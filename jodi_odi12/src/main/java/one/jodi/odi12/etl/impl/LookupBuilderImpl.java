...
public class LookupBuilderImpl implements LookupBuilder {
    ...
    private final static Logger logger = LogManager.getLogger(LookupBuilderImpl.class);
    private final static String LookupMultipleMatchedRows = "jodi.lookup.multiple_match_rows";
    ...
    @Inject
    protected LookupBuilderImpl(final OdiTransformationAccessStrategy<MapRootContainer, Dataset, DatastoreComponent, ReusableMappingComponent, IMapComponent, OdiContext, ILogicalSchema> odiAccessStrategy,
    ...
    public void addLookups(final MapRootContainer mapping, final List<Source> sources, final boolean journalized,
                           final List<JoinComponent> joiners, final EtlOperators etlOperators)
            throws AdapterException, MappingException, TransformationAccessStrategyException,
        ...
        for (Source source : sources) {
            ...
            if (source.getLookups() != null && sources.size() == 1) {
                if (source.getFilter() != null && source.getFilter().length() > 2) {
                    ...
                    firstComponent = odiAccessStrategy.getComponentByName(mapping,
                            ComponentPrefixType.FILTER.getAbbreviation() + "_" + source.getComponentName());
                    logger.debug("firstComponent" + firstComponent.getName());
                } else if (source.getParent().getSources().size() == 1) {
                    ...
                    if (source.getFlows().size() > 0) {
                        ...
                        firstComponent = odiAccessStrategy.findSourceComponent(mapping, source.getParent().getParent(),
                                source);
                    }
                ...
                int dataSetNumber = sources.get(0).getParent().getDataSetNumber();
            ...
            for (int lookupCounter = 0; lookupCounter < source.getLookups().size(); lookupCounter++) {
                Lookup lookup = source.getLookups().get(lookupCounter);
                datasetLookupCounter++;
                logger.debug("firstComponent was:" + firstComponent.getName());
                if (lookupCounter == 0) {
                    firstComponent = addLookupComponent(mapping, firstComponent, lookup, datasetLookupCounter,
                            etlOperators);
     ...
     */
    private LookupComponent addLookupComponent(final MapRootContainer mapping, IMapComponent firstComponent, Lookup lookup,
                                               final int counter, final EtlOperators etlOperators)
            throws AdapterException, MappingException, TransformationAccessStrategyException,
            ResourceNotFoundException, ResourceFoundAmbiguouslyException {
        IMapComponent secondComponent = null;
        String alias = lookup.getComponentName();
        if (!lookup.isTemporary()) {
            OdiDataStore lookupDataStore = odiAccessStrategy.findDataStore(lookup.getLookupDataStore(),
                    lookup.getModel());
            assert (lookupDataStore != null);
            secondComponent = datastoreBuilder.createComponent(mapping, lookupDataStore, false);
        } else {
            String folder = lookup.getParent().getParent().getParent().getFolderName();
            ReusableMapping lookupDataStore = (ReusableMapping) odiAccessStrategy
                    .findMappingsByName(lookup.getLookupDataStore(), folder, properties.getProjectCode());
            assert (lookupDataStore != null);
            secondComponent = datastoreBuilder.createComponent(mapping, lookupDataStore, false);
        }
     ...
     *                MappingException
     */
    private LookupComponent createLookup(final MapRootContainer mapping, final Lookup lookup, final IMapComponent driver,
                                         final IMapComponent lookupComponent, final int counter) throws AdapterException, MappingException {

        String name = ComponentPrefixType.DATASET.getAbbreviation() + lookup.getParent().getParent().getDataSetNumber() + "_"
        ...
        if (!lookup.isTemporary() && !lookup.getParent().isTemporary() && !lookup.getParent().getParent().getParent().isTemporary()) {
            String lookupType = LookupComponent.LookupType.ANY_ROW.name();
            if (properties.getPropertyKeys().contains(LookupMultipleMatchedRows)) {
                lookupType = properties.getProperty(LookupMultipleMatchedRows).trim();
            }
            if (lookup.getLookupType() != null && !lookup.getLookupType().equals(LookupTypeEnum.SCALAR)) {
                lookupType = lookup.getLookupType().name();
            }
        ...
        if (lookup.getDefaultRowColumns().size() > 0) {
            leftJoin.setNoMatchRows(NO_MATCH_ROWS.DEFAULT_VALUES);

            lookup.getDefaultRowColumns().forEach((k, v) -> {
                try {
                    leftJoin.setLookupAttributeDefaultValue(k, lookup.getParent().getParent().translateExpression(v));
        ...
        leftJoin.addJoinSource(lookupComponent, lookup.getParent().getParent().translateExpression(lookup.getJoin()));
        ...
        leftJoin.setJoinConditionText(leftJoin.getJoinConditionText().replace("1=1 AND ", ""));
        return leftJoin;
    }
