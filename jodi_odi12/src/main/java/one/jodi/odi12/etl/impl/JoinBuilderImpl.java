...
public class JoinBuilderImpl implements JoinBuilder {
    ...
    private final static Logger logger = LogManager.getLogger(JoinBuilderImpl.class);
    private final OdiTransformationAccessStrategy<MapRootContainer, Dataset,
            DatastoreComponent,
            ReusableMappingComponent, IMapComponent,
            OdiContext, ILogicalSchema> odiAccessStrategy;
    ...
    @Inject
    protected JoinBuilderImpl(final OdiTransformationAccessStrategy<MapRootContainer, Dataset,
            DatastoreComponent, ReusableMappingComponent,
            IMapComponent, OdiContext, ILogicalSchema>
                              ...
                              final LookupBuilder lookupBuilder,
    ...
    @Override
    public void addDatasets(final MapRootContainer mapping,
                            final Transformation transformation,
                            final boolean journalized,
                            final List<FilterComponent> filterComponentsss,
                            final EtlOperators etlOperators,
                            final boolean useExpressions)
            throws AdapterException, MapPhysicalException,
            MappingException, TransformationAccessStrategyException,
            TransformationException, ResourceNotFoundException,
        ...
        for (int currentDatasetIdx = 0; currentDatasetIdx < transformation.getDatasets().size(); currentDatasetIdx++) {
            ...
            for (Source joinedTable : ds.findJoinedSourcesInDataset()) {
                ...
                if (joinedTable.isTemporary()) {
                    String folder = joinedTable.getParent().getParent().getFolderName();
                    ReusableMapping joinedreusableMapping = (ReusableMapping) odiAccessStrategy
                            .findMappingsByName(joinedTable.getName(), folder, properties.getProjectCode());
    ...
    private void addJoins(final MapRootContainer mapping, final Transformation transformation,
                          final boolean journalized, final one.jodi.etl.internalmodel.Dataset ds, final int currentDatasetIdx,
                          final List<FilterComponent> filterComponents, final EtlOperators etlOperators) throws AdapterException,
        ...
        for (int counter = 0; counter < ds.getSources().size(); counter++) {
            Source filteredSource = ds.getSources().get(counter);
            filterBuilder.addFilter(mapping, filteredSource, journalized, etlOperators);
            if (counter == 0) {
                Source firstSource = ds.getSources().get(counter);
                // find which source the seconds source is joined to.
                if (!firstSource.getFlows().isEmpty()) {
                    firstComponent = etlOperators.getFlows(firstSource).get(etlOperators.getFlows(firstSource).size() - 1);
                } else if (firstSource.getFilter() != null && firstSource.getFilter().length() > 1) {
                    firstComponent = etlOperators
                            .getFilterComponentsByKey(FilterBuilder.getFilterKeyForSource(firstSource, journalized));
                } else {
                    firstComponent = odiAccessStrategy.findSourceComponent(mapping, transformation, firstSource);
                }
                assert (firstComponent != null);
            } else {
                Source secondSource = ds.getSources().get(counter);
                ...
                if (secondSource.getJoinType().equals(JoinTypeEnum.INNER)) {
                ...
                } else if (secondSource.getJoinType().equals(JoinTypeEnum.LEFT_OUTER)) {
                    innerJoin.setJoinConditionText("1=1");
                    innerJoin.addJoinSource(firstComponent, "1=1");
                    innerJoin.setJoinType(JoinComponent.JOIN_TYPE_LEFT_OUTER);
                } else if (secondSource.getJoinType().equals(JoinTypeEnum.FULL)) {
                    innerJoin.setJoinConditionText("1=1");
                    innerJoin.addJoinSource(firstComponent, "1=1");
                    innerJoin.setJoinType(JoinComponent.JOIN_TYPE_FULL_OUTER);
                } else if (secondSource.getJoinType().equals(JoinTypeEnum.CROSS)) {
                    innerJoin.addJoinSource(firstComponent, null);
                    innerJoin.setJoinType(JoinComponent.JOIN_TYPE_CROSS);
                } else if (secondSource.getJoinType().equals(JoinTypeEnum.NATURAL)) {
                    innerJoin.addJoinSource(firstComponent, "1=1");
                    innerJoin.setJoinType(JoinComponent.JOIN_TYPE_NATURAL);
                }
                assert (firstComponent != null);
                if (secondSource.getFilter() != null && secondSource.getFilter().length() > 1) {
                    secondComponent = etlOperators
                            .getFilterComponentsByKey(FilterBuilder.getFilterKeyForSource(secondSource, journalized));
                    assert (secondComponent != null);
                } else if (!secondSource.getFlows().isEmpty()) {
                    //pick up last Flow item
                    secondComponent = etlOperators.getFlows(secondSource).get(etlOperators.getFlows(secondSource).size() - 1);
                } else {
                    secondComponent = odiAccessStrategy.findSourceComponent(mapping, transformation, secondSource);
                    assert (secondComponent != null);
                }
                if (innerJoin != null) {
                    String join = secondSource.getParent().translateExpression(secondSource.getJoin());
                    if (secondSource.getJoinType().equals(JoinTypeEnum.CROSS)
                            || secondSource.getJoinType().equals(JoinTypeEnum.NATURAL)) {
                        join = null;
                    }
                    ...
                    etlOperators.addJoiner(getNextIncrementalValue(etlOperators.getJoiners(currentDatasetIdx + 1)), innerJoin);
                }

                if (secondComponent.getComponentTypeName().equals("DATASTORE")
                        || secondComponent.getComponentTypeName().equals("REUSABLEMAPPING")
                        || secondComponent.getComponentTypeName().equals("FILE")) {
                    if (secondSource.getJoinExecutionLocation() != null) {
                        innerJoin.setExecuteOnHint(
                                mapFromExecutionLocationType(secondSource.getJoinExecutionLocation()));
                    }
                ...
                innerJoin.setJoinConditionText(innerJoin.getJoinConditionText().replace("1=1 AND ", ""));
                // end cleanup
    ...
    protected JoinComponent createJoinComponent(MapRootContainer mapping, int datasetIndex, int joinIndex, String dataStoreName) throws MappingException {
        return new JoinComponent(mapping, ComponentPrefixType.DATASET.getAbbreviation()
                + datasetIndex + "_" + ComponentPrefixType.JOIN.getAbbreviation() + joinIndex + "_" + dataStoreName);
    }
}
