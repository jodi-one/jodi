...
public class KMBuilderImpl implements KMBuilder {
    private final static Logger logger = LogManager.getLogger(KMBuilderImpl.class);
    private final static String ERROR_MESSAGE_09997 = "%3$s: Couldn't set update key for mapping '%1$s' of datastore '%2$s'.";
    ...
    @Inject
    protected KMBuilderImpl(
            final OdiCommon odiCommon,
     ...
     */
    @Override
    public void setLKM(final MapRootContainer mapping, final Transformation transformation)
            throws MappingException, AdapterException, GenerationException, TransformationException {

        if (!(mapping instanceof Mapping)) {
            return;
        }
        String IKMname = properties.getProperty(transformation.getMappings().getIkm().getName() + ".name");
        boolean globalIKM = properties.getPropertyKeys().contains(transformation.getMappings().getIkm().getName() + ".global") ?
                Boolean.valueOf(properties.getProperty(transformation.getMappings().getIkm().getName() + ".global")) : false;

        OdiKM<?> ikm = !globalIKM ? odiCommon.findIKMByName(IKMname, properties.getProjectCode()) : odiCommon.findIKMByName(IKMname);
        ...
        for (Dataset dataset : transformation.getDatasets()) {
            for (Source source : dataset.getSources()) {
                String name = properties.getProperty(source.getLkm().getName() + ".name");
                String globalProperty = source.getLkm().getName() + ".global";
                boolean global = properties.getPropertyKeys().contains(globalProperty) ?
                        Boolean.valueOf(properties.getProperty(globalProperty)) : false;
                OdiKM<?> lkm = !global ? odiCommon.findLKMByName(name, properties.getProjectCode()) : odiCommon.findLKMByName(name);
                ...
                for (MapPhysicalDesign physicalDesign : ((Mapping) mapping).getPhysicalDesigns()) {
                    for (MapPhysicalNode node : physicalDesign.getAllAPNodes()) {
                        ...
                        if (source.getFilter() != null && source.getFilter().length() > 2 && source.getFilterExecutionLocation() != null && source.getFilterExecutionLocation().equals(ExecutionLocationtypeEnum.SOURCE)) {
                            alias = ComponentPrefixType.FILTER.getAbbreviation() + "_" + ComponentPrefixType.DATASET.getAbbreviation() + source.getParent().getDataSetNumber() + alias + "_AP";
                        } else {
                            alias = ComponentPrefixType.DATASET.getAbbreviation() + source.getParent().getDataSetNumber() + alias + "_AP";
                        }
                        if (alias.length() > 30) {
                        ...
                        if (ikm.isMultiConnectionSupported()) {
                            ...
                            logger.debug(String.format("LKM set to '%1$s' and multiconnect supported is %2$s for source %3$s.",
                        ...
                        assert node.getLKM().getName().equalsIgnoreCase(name) : "Source LKM not set to " + name + " but to name " + node.getLKM().getName();
                    }
                }
            }
        }
    ...
    public void setIKM(final MapRootContainer mapping, final Transformation transformation,
                       final List<IMapComponent> targetComponents)
            throws AdapterException, MappingException, GenerationException, TransformationException {
        String name = properties.getProperty(transformation.getMappings().getIkm().getName() + ".name");
        boolean global = properties.getPropertyKeys().contains(transformation.getMappings().getIkm().getName() + ".global") ?
                Boolean.valueOf(properties.getProperty(transformation.getMappings().getIkm().getName() + ".global")) : false;
        OdiKM<?> ikm = !global ? odiCommon.findIKMByName(name, properties.getProjectCode()) : odiCommon.findIKMByName(name);
        ...
        if (targetComponents.get(0) instanceof DatastoreComponent) {
            for (IKey key : ((DatastoreComponent) targetComponents.get(0)).getBoundDataStore().getKeys()) {
            ...
            if (transformation.getMappings().hasUpdateKeys()) {
                // there are update keys specified in xml so we use that
                List<String> columnNames =
                        transformation.getMappings().getTargetColumns().stream()
                                .filter(Targetcolumn::isUpdateKey)
                                .map(Targetcolumn::getName)
                                .collect(Collectors.toList());
                @SuppressWarnings("unchecked")
                IKey key = getKeyFromExplicitlySpecifiedKeys(
                        (Collection<IKey>) ((DatastoreComponent) targetComponents.get(0)).getBoundDataStore().getKeys(),
                        columnNames);
                if (key != null) {
                    ((DatastoreComponent) targetComponents.get(0)).setUpdateKey(key);
                    foundAtLeastOneUpldateKey = true;
                    ...
                    String message = errorWarningMessages.formatMessage(9997, ERROR_MESSAGE_09997, this.getClass(),
                            transformation.getName(), targetComponents.get(0).getName(), transformation.getPackageSequence());
                    errorWarningMessages.addMessage(transformation.getPackageSequence(), message, MESSAGE_TYPE.WARNINGS);
                }
            }
            ((DatastoreComponent) targetComponents.get(0))
                    .setIntegrationType(ikm.getIntegrationType());
            logger.debug(ikm.getIntegrationType());
        }
        if (!foundAtLeastOneUpldateKey && transformation.getMappings().hasUpdateKeys()) {
            String message = errorWarningMessages.formatMessage(9997, ERROR_MESSAGE_09997, this.getClass(),
                    transformation.getName(), targetComponents.get(0).getName(), transformation.getPackageSequence());
            errorWarningMessages.addMessage(transformation.getPackageSequence(), message, MESSAGE_TYPE.WARNINGS);
            ...
            for (ExecutionUnit teu : pd.getTargetExecutionUnits()) {
                for (MapPhysicalNode target : teu.getTargetNodes()) {
                    ...
                    applyKnowledgeModuleConfigurationSettings(mapping,
                            ...
                            transformation.getMappings().getIkm().getOptions(), target);
                }
            }
        }
    }
    ...
    private void setUpdateKeyToUpdateTrue(final IKey key, final Transformation transformation,
        ...
        for (IColumn col : key.getColumns()) {
            for (MapAttribute ma : targetComponents.get(0).getAttributes()) {
                if (ma.getName().equals(col.getName())) {
    ...
    private IKey getKeyFromExplicitlySpecifiedKeys(Collection<IKey> keys, List<String> columnNames) {
        for (IKey key : keys) {
            Set<String> keyColumnsSet = key.getColumns().stream()
     ...
     */
    @Override
    public void setCKM(final MapRootContainer mapping, final Transformation transformation)
            throws AdapterException, MappingException, GenerationException, TransformationException {
        if (transformation.getMappings().getCkm() != null) {
            String name = properties.getProperty(transformation.getMappings().getCkm().getName() + ".name");
            boolean global = properties.getPropertyKeys().contains(transformation.getMappings().getCkm().getName() + ".global") ?
                    Boolean.valueOf(properties.getProperty(transformation.getMappings().getCkm().getName() + ".global")) : false;

            OdiKM<?> ckm = !global ? odiCommon.findCKMByName(name, properties.getProjectCode()) : odiCommon.findCKMByName(name);
            if (mapping instanceof Mapping) {
                for (MapPhysicalDesign pd : ((Mapping) mapping).getPhysicalDesigns()) {
                    for (ExecutionUnit teu : pd.getTargetExecutionUnits()) {
                        for (MapPhysicalNode target : teu.getTargetNodes()) {
                            ...
                            //
                            applyKnowledgeModuleConfigurationSettings(mapping, KMType.CKM,
                                    transformation.getMappings().getCkm().getOptions(), target);
                        }
                    }
                }
            }
    ...
    private void applyKnowledgeModuleConfigurationSettings(final MapRootContainer mapping, final KMType km,
                                                           final Map<String, String> options, final MapPhysicalNode target)
            throws AdapterException, MappingException, GenerationException, TransformationException {
        assert (target != null);
        if (mapping instanceof Mapping && km.equals(oracle.odi.domain.adapter.project.IKnowledgeModule.KMType.IKM)) {
            for (Entry<String, String> option : options.entrySet()) {
                if (target.getIKM() != null && target.getIKM().getName() != null) {
                    validateKMOption(target.getIKM().getName(), option.getKey(), target.getIKMOptionValues());
                }
                if (option.getValue() != null) {
                    target.getIKMOptionValue(option.getKey()).setValue(option.getValue());
                }
            }
        }
        if (mapping instanceof Mapping && km.equals(oracle.odi.domain.adapter.project.IKnowledgeModule.KMType.CKM)) {
            for (Entry<String, String> option : options.entrySet()) {
                if (target.getCheckKM() != null && target.getCheckKMOptionValues() != null
                        && target.getCheckKMOptionValues().size() > 0) {
                    validateKMOption(target.getCheckKM().getName(), option.getKey(), target.getCheckKMOptionValues());
                    target.getCheckKMOptionValue(option.getKey()).setValue(option.getValue());
                }
            }
        }
    }
     ...
     */
    private void validateKMOption(final String KMname, final String key, final List<IOptionValue> ikmOptionValues)
            throws TransformationException {
        boolean found = false;
        for (IOptionValue optionVal : ikmOptionValues) {
            if (optionVal.getName().equals(key)) {
                found = true;
            }
        }
        if (!found) {
    ...
    private void applyKnowledgeModuleConfigurationSettings(final MapRootContainer mapping, final KMType lkm,
                                                           final List<IOptionValue> lkmOptionValues, final MapPhysicalNode node, final Source source,
                                                           @SuppressWarnings("rawtypes") final OdiKM lkmModule)
            throws AdapterException, MappingException, TransformationException {
        for (String key : source.getLkm().getOptions().keySet()) {
            validateKMOption(lkmModule.getName(), key, node.getLKMOptionValues());
            Object value = source.getLkm().getOptions().get(key);
            if (value.toString().toLowerCase().equals("true") || value.toString().toLowerCase().equals("false")) {
                Boolean valu = Boolean.valueOf(node.getLKMOptionValue(key).toString());
                ...
                String valu = node.getLKMOptionValue(key).toString();
                valu = value.toString();
                for (IOptionValue option : node.getLKMOptionValues()) {
                    if (option.getName().equals(key)) {
                        logger.debug("Set KMOption: " + key + " = " + value);
