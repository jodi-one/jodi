...
public class OdiTableServiceImpl implements TableServiceProvider {
    ...
    private final static Logger logger = LogManager.getLogger(OdiTableServiceImpl.class);
    private static final String ERROR_MESSAGE_01210 =
            "Matching column '%s' has been identified but not found in " +
                    "datastore '%s'.";
    private static final String ERROR_MESSAGE_01220 =
            "ERROR: deleting Key: %s try deleting any packages that include " +
                    "this interface. %s.";
    private static final String ERROR_MESSAGE_01230 =
            "Cannot find JKM with name '%1$s' in project '%2$s', is it " +
                    "imported?";
    private static final String ERROR_MESSAGE_06000 =
            "OdiReference has no index for column: %s.%s";
    private static final String ERROR_MESSAGE_06001 =
            "OdiReference %s of table %s is not defined in database.";
    private static final String ERROR_MESSAGE_06002 =
            "OdiReference %s of table %s doens't have primary datastore.";
    ...
    @Inject
    protected OdiTableServiceImpl(OdiInstance instance,
                                  SchemaMetaDataProvider odiUtils, final JodiProperties properties,
    ...
    public void alterTables(List<TableDefaultBehaviors> tablesToChange) {
        ...
        Collection<OdiDataStore> odiDetailDatastore = odiInstance
                .getTransactionalEntityManager().findAll(OdiDataStore.class);

        for (TableDefaultBehaviors tdb : tablesToChange) {

            OdiDataStore matchingOdiDataStore = getMatchingDataStore(tdb,
                    odiDetailDatastore);
            assert (matchingOdiDataStore != null);
            assert (tdb.getDefaultAlias() != null);
            assert (matchingOdiDataStore.getName() != null);
            if (tdb.getDefaultAlias().equals(tdb.getTableName())) {
            ...
            odiInstance.getTransactionalEntityManager().merge(matchingOdiDataStore);

            OdiColumn matchingColumn = null;
            for (ColumnDefaultBehaviors cdb : tdb.getColumnDefaultBehaviors()) {
                assert (!cdb.getColumnName().isEmpty()) : "Model error";
                matchingColumn = getMatchingOdiColumn(matchingOdiDataStore,
                        cdb.getColumnName());
                if (matchingColumn == null) {
                    String message = errorWarningMessages.formatMessage(1210,
                            ERROR_MESSAGE_01210, this.getClass(),
                            cdb.getColumnName(), matchingOdiDataStore);
                    errorWarningMessages.addMessage(message, MESSAGE_TYPE.ERRORS);
                    throw new AssertionError(message);
                }

                if (cdb.getScdType().equals("SURROGATE_KEY")) {
            ...
            if (tdb.isConnectorModel()) {
                Collection<OdiReference> odiReferences =
                        findReferenceByPrimaryDataStoreId(matchingOdiDataStore
                                .getDataStoreId());

                logger.info("odiReferences: " + odiReferences.size());
                for (OdiReference odiReference : odiReferences) {
                    alterReferenceInConnector(matchingOdiDataStore,
                            odiReference);
                }
            }
        }
    ...
    private OlapType convertToOdiCoding(
            TableDefaultBehaviors.OlapType olapType) {
        OlapType value = null;
        if (olapType == TableDefaultBehaviors
                .OlapType.FACT) {
            value = OlapType.FACT_TABLE;
        } else if (olapType == TableDefaultBehaviors
                .OlapType.DIMENSION) {
            value = OlapType.DIMENSION;
        } else if (olapType == TableDefaultBehaviors
                .OlapType.SLOWLY_CHANGING_DIMENSION) {
            value = OlapType.SLOWLY_CHANGING_DIMENSION;
        }
        return value;
    }

    private void alterKeyDefinedInDatabase(OdiKey odiKey,
                                           OdiDataStore matchingOdiDataStore, TableDefaultBehaviors tdb) {

        for (ColumnDefaultBehaviors cdb : tdb.getColumnDefaultBehaviors()) {
            if (!cdb.isInDatabase()) {
                logger.debug("ColumnName: " + cdb.getColumnName()
                        + " is not defined in the database altering it.");
            }
            odiKey.setInDatabase(true);
            odiInstance.getTransactionalEntityManager().merge(
                    matchingOdiDataStore);
        }
    }

    private OdiDataStore getMatchingDataStore(TableDefaultBehaviors tdb,
                                              Collection<OdiDataStore> odiDetailDatastore) {
        OdiDataStore matchingOdiDataStore = null;
        for (OdiDataStore odiDataStore : odiDetailDatastore) {
            if (odiDataStore.getName().equals(tdb.getTableName())) {
                matchingOdiDataStore = odiDataStore;
                logger.debug("Get matching dataStore, matchingOdiDataStore: "
                        + matchingOdiDataStore);
            }
        }
     ...
     * @return
     */
    private OdiColumn getMatchingOdiColumn(OdiDataStore odiDataStore,
                                           String columnName) {
        OdiColumn matchingColumn = null;
        for (OdiColumn odiColumn : odiDataStore.getColumns()) {
            if (odiColumn.getName().equals(columnName)) {
                matchingColumn = odiColumn;
                logger.debug("Get matching column, matchingColumn: "
                        + matchingColumn);
            }
        }
    ...
    @Override
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public void setCDCDescriptor(String dataStoreName, String dataModelCode,
                                 int order) {
        IOdiDataStoreFinder finder = (IOdiDataStoreFinder) odiInstance
                .getTransactionalEntityManager().getFinder(OdiDataStore.class);
        OdiDataStore odiDataStore = finder.findByName(dataStoreName,
                dataModelCode);
        logger.debug(String.format(
                "Setting CDC descriptor to true for datastore '%1$s'.",
                dataStoreName));
        odiDataStore.setCdcDescriptor(new OdiDataStore.CdcDescriptor(true,
                order));
        odiInstance.getTransactionalEntityManager().merge(odiDataStore);
    }
    ...
    public void resetCDCDescriptor() {
        IOdiDataStoreFinder finder = (IOdiDataStoreFinder) odiInstance
                .getTransactionalEntityManager().getFinder(OdiDataStore.class);
        @SuppressWarnings("unchecked")
        Collection<OdiDataStore> odiDataStores = finder.findAll();
        for (OdiDataStore dataStore : odiDataStores) {
            logger.debug(String.format(
                    "Setting CDC descriptor to false for datastore '%1$s'.",
                    dataStore.getName()));
            dataStore
                    .setCdcDescriptor(new OdiDataStore.CdcDescriptor(false, 0));
            odiInstance.getTransactionalEntityManager().merge(dataStore);
        }
    }
    ...
    @Override
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public void setSubscriber(String dataStoreName, String dataModelCode,
        ...
        OdiDataStore odiDataStore = finder.findByName(dataStoreName,
                dataModelCode);
        logger.debug(String.format(
                "Setting subscriber to for datastore '%1$s'.", dataStoreName));
        odiInstance.getTransactionalEntityManager().merge(odiDataStore);
    }
    ...
    public void setJKMOptions(String modelCode, Map<String, Object> jkmOptions) {
        OdiModel odiModel = ((IOdiModelFinder) odiInstance
        ...
        for (IOptionValue jkmOption : odiModel.getJKMOptions()) {
            for (Entry<String, Object> entryOption : jkmOptions.entrySet()) {
                if (entryOption.getKey().equalsIgnoreCase(jkmOption.getName())) {
                    if (Boolean.TRUE.equals(entryOption.getValue()) ||
                            Boolean.FALSE.equals(entryOption.getValue())) {
                        odiModel.getJKMOptions().get(jkmCounter)
                                .setValue(Boolean.parseBoolean(entryOption.getValue() + ""));
                        logger.debug("JKMOption:" + entryOption.getKey() +
                                " set to value:" + entryOption.getValue());
                    } else {
                        odiModel.getJKMOptions().get(jkmCounter)
    ...
    private void alterReferenceInConnector(OdiDataStore odiDatastore,
                                           OdiReference odiReference) {
        logger.debug("Removing Foreign Key: " + odiReference.getName());
        odiInstance.getTransactionalEntityManager().remove(odiReference);
    }
    ...
    public void checkTables() {
        @SuppressWarnings("unchecked")
        Collection<OdiReference> odiReferences = odiInstance
                .getTransactionalEntityManager().findAll(OdiReference.class);
        for (OdiReference odiReference : odiReferences) {
            ...
            for (String dmp : properties.getPropertyList(JodiConstants.DATA_MART_PREFIX)) {
                if (odiReference.getPrimaryDataStore() == null) {
                    String message = errorWarningMessages.formatMessage(6001,
                            ERROR_MESSAGE_06002, this.getClass(),
                            odiReference.getName(), odiReference
                                    .getForeignDataStore().getName());
                    errorWarningMessages.addMessage(message, MESSAGE_TYPE.WARNINGS);
                    continue;
                }
                if (odiReference.getPrimaryDataStore().getName().startsWith(dmp)) {
                    hasDataMartPrefix = true;
                }
            }
            if (odiReference.getPrimaryDataStore() != null && hasDataMartPrefix) {
                Collection<ReferenceColumn> refcolumns = odiReference
                        .getReferenceColumns();
                for (ReferenceColumn refCol : refcolumns) {
                    ...
                    if (!found) {
                        String message = errorWarningMessages.formatMessage(6000,
                                ERROR_MESSAGE_06000, this.getClass(),
                                odiReference.getForeignDataStore().getName(),
                                odiRefColumn.getName());
                        errorWarningMessages.addMessage(message, MESSAGE_TYPE.WARNINGS);
                    }
                }
                if (odiReference.getReferenceType() !=
                        OdiReference.ReferenceType.DB_REFERENCE) {
                    String message = errorWarningMessages.formatMessage(6001,
                            ERROR_MESSAGE_06001, this.getClass(),
                            odiReference.getName(), odiReference
                                    .getForeignDataStore().getName());
                    errorWarningMessages.addMessage(message, MESSAGE_TYPE.WARNINGS);
                }
    ...
    private Collection<OdiReference> findReferenceByPrimaryDataStoreId(
            final Number dataStoreId) {
        return ((IOdiReferenceFinder) odiInstance
                .getTransactionalEntityManager().getFinder(OdiReference.class))
                .findByPrimaryDataStore(dataStoreId);
    }

    private OdiColumn findColumn(final OdiDataStore odiDataStore,
                                 final ColumnDefaultBehaviors c) {
        OdiColumn found = null;
        for (OdiColumn column : odiDataStore.getColumns()) {
            if (column.getName().equalsIgnoreCase(c.getColumnName())) {
                found = column;
                break;
            }
        }
    ...
    private void setSCDType2Behavior(final TableDefaultBehaviors tdb,
                                     final Map<String, OdiDataStore> datastores) {
        ...
        odiDataStore.setOlapType(OdiDataStore.OlapType
        ...
        for (ColumnDefaultBehaviors c : tdb.getColumnDefaultBehaviors()) {
            ...
            logger.debug(odiDataStore.getName() + ":"
                    + odiColumn.getName() + ":"
                    + odiDataStore.getOlapType() + ":"
    ...
    public void alterSCDTables(List<TableDefaultBehaviors> tablesToChange) {
        ...
        for (OdiDataStore ods : odiDetailDatastore) {
            datastores.put(ods.getModel().getCode() + "." + ods.getName(), ods);
        }

        tablesToChange.stream().filter(tdb -> tdb.getOlapType() ==
                TableDefaultBehaviors.OlapType.SLOWLY_CHANGING_DIMENSION)
                .forEach(tdb -> setSCDType2Behavior(tdb,
                        datastores));
    }
    ...
    public void deleteReferencesByModel(String odiModel) {
        try {
            ...
            String message = errorWarningMessages.formatMessage(1220,
                    ERROR_MESSAGE_01220, this.getClass(),
                    odiModel, re.getMessage());
            errorWarningMessages.addMessage(message, MESSAGE_TYPE.ERRORS);
    ...
    private void deleteReferenceByModel(String name) {
        @SuppressWarnings("unchecked")
        Collection<OdiReference> odireferences = ((IOdiReferenceFinder) odiInstance
                .getTransactionalEntityManager().getFinder(OdiReference.class))
                .findAll();
        ...
        if (!odireferences.isEmpty()) {
            for (OdiReference odireference : odireferences) {
                if (odireference == null
                        || odireference.getForeignDataStore() == null
                        || odireference.getForeignDataStore().getModel() == null
                        || odireference.getForeignDataStore().getModel().getCode() == null) {
                    continue;
                }
                if (odireference.getForeignDataStore().getModel().getCode()
                        .equals(name)) {
                    logger.info("Model: " + name + ":reference:"
                            + odireference.getForeignDataStore().getModel().getCode());
                    odiInstance.getTransactionalEntityManager().remove(odireference);

                    logger.info("Removed reference: " + odireference.getName()
                            + " from model: " + name + " from datastore:"
                            + odireference.getForeignDataStore().getName());
    ...
    private void deleteIndexesByModel(final String model) {
        ...
        for (OdiDataStore ds : allDs) {
            ...
            for (OdiKey key : ds.getKeys()) {
                if (key != null &&
                        key.getKeyType() != null &&
                        key.getKeyType().equals(oracle.odi.domain.model.OdiKey.KeyType.INDEX)) {
                    logger.info("Index: " + key.getName() + " :reference:"
                            + ds.getModel().getCode() + "." + ds.getName());
                    odiInstance.getTransactionalEntityManager().remove(key);
                }
            }
        }
        // tm.commit(txnStatus);
    ...
    public void setJKM(String modelCode, String jkmName) {
        OdiModel odiModel = ((IOdiModelFinder) odiInstance
                .getTransactionalEntityManager().getFinder(OdiModel.class))
                .findByCode(modelCode);
        Collection<OdiJKM> odiJKMs = ((IOdiJKMFinder) odiInstance
                .getTransactionalEntityManager().getFinder(OdiJKM.class))
                .findByName(jkmName, properties.getProjectCode());
        if (odiJKMs.size() != 1) {
            String message = errorWarningMessages.formatMessage(1230,
                    ERROR_MESSAGE_01230, this.getClass(), jkmName,
                    properties.getProjectCode());
            errorWarningMessages.addMessage(message, MESSAGE_TYPE.ERRORS);
            throw new RuntimeException(message);
        }
        OdiJKM odiJKM = odiJKMs.iterator().next();
        odiModel.setJKM(odiJKM);
        odiInstance.getTransactionalEntityManager().persist(odiModel);
        logger.info(String.format(
                "JKM '%1$s' set for model '%2$s' in project '%3$s'.",
                odiJKM.getName(), modelCode, properties.getProjectCode()));
    }
    ...
    public void resetJKMs() {
        ...
        for (ModelProperties modelp : models) {
            OdiModel odiModel = ((IOdiModelFinder) odiInstance
                    .getTransactionalEntityManager().getFinder(OdiModel.class))
                    .findByCode(modelp.getCode());
            odiModel.setJKM(null);
            odiInstance.getTransactionalEntityManager().persist(odiModel);
            logger.info(String.format(
                    "Resetting JKMs  for model '%1$s' in project '%2$s'.",
                    modelp.getCode(), properties.getProjectCode()));
