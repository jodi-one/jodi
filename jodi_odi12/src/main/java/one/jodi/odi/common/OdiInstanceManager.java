...
public class OdiInstanceManager implements Provider<OdiInstance> {
    ...
    private static final Logger logger = LogManager.getLogger(OdiInstanceManager.class);
    private static final int MAX_SIZE = 3;
    private static final Map<Integer, OdiInstance> allOdiInstances = new DeferredCleanupCache(MAX_SIZE);
    ...
    static {
        Runtime.getRuntime()
               .addShutdownHook(new Thread() {
                   ...
                   @Override
                   public void run() {
                       allOdiInstances.values()
                                      .stream()
                                      .filter(oi -> !oi.isClosed())
                                      .forEach(oi -> {
                                          ...
                                          try {
                                              ...
                                              LogManager.getLogger(OdiInstanceManager.class)
                                                        .info("Exception generated closing ODI instance " + oi, e);
                                          }
    ...
    @Inject
    public OdiInstanceManager(final JodiProperties properties, @MasterPassword final String masterPassword,
    ...
    public OdiInstance init() {
        ...
        int currentHash = getCurrentHash(odiRepoUrl, odiRepoDbDriver, odiMasterUser, odiWorkRepo, odiUser);
        OdiInstance odiInst = allOdiInstances.get(currentHash);
        if (odiInst == null) {
            logger.info(
                    String.format("create new ODI Instance for ConnectionUrl: %1$s and OdiMasterUser: %2$s", odiRepoUrl,
                                  odiMasterUser));
            MasterRepositoryDbInfo masterInfo = new MasterRepositoryDbInfo(odiRepoUrl, odiRepoDbDriver, odiMasterUser,
                                                                           odiMasterPassword.toCharArray(),
                                                                           new PoolingAttributes());
            WorkRepositoryDbInfo workInfo = new WorkRepositoryDbInfo(odiWorkRepo, new PoolingAttributes());
            odiInst = OdiInstance.createInstance(new OdiInstanceConfig(masterInfo, workInfo));
            ...
            odiInst.getSecurityManager()
                   .setCurrentThreadAuthentication(auth);
    ...
    private int getCurrentHash(String odiRepoUrl, String odiRepoDbDriver, String odiMasterUser, String odiWorkRepo,
                               String odiUser) {
...
class DeferredCleanupCache extends LinkedHashMap<Integer, OdiInstance> {
    ...
    protected boolean removeEldestEntry(Map.Entry<Integer, OdiInstance> eldest) {
        ...
        if (remove) {
            ...
            eldest.getValue()
                  .close();
            logger.debug(String.format("ODI Instance %1$s closed and ready for garbage collection", eldest.getValue()
                                                                                                          .toString()));
        }
