...
public class OdiVersion implements EtlSubSystemVersion {
    public final static Logger logger = LogManager.getLogger(OdiVersion.class);

    private static final String ODI_INTERFACE_CLASS =
            "/oracle/odi/domain/project/OdiInterface.class";
    ...
    private int getGlobalVersion(final OdiInstance odiInstance) {
        int version = -1;
        if (odiInstance.getWorkRepository().getLegacyConnectionDef()
                .getProductVersion().isBefore11G())
            version = 10;
        else if (odiInstance.getWorkRepository().getLegacyConnectionDef()
                .getProductVersion().toString().startsWith("11."))
            version = 11;
        else if (odiInstance.getWorkRepository().getLegacyConnectionDef()
                .getProductVersion().toString().startsWith("12."))
            version = 12;
        else
            version = -1;
