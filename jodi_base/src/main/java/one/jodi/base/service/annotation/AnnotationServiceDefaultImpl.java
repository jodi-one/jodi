...
public class AnnotationServiceDefaultImpl implements AnnotationService {
    ...
    private final static Logger logger =
            LogManager.getLogger(AnnotationServiceDefaultImpl.class);
    ...
    private static final String ERROR_MESSAGE_80310 =
            "Programming error when accessing table '%1$s' annotation.";

    private static final String ERROR_MESSAGE_80320 =
            "Malformed embedded annotation in table '%1$s' of schema '%2$s'.";

    private static final String ERROR_MESSAGE_80330 =
            "Malformed embedded annotation in column '%1$s' of table '%2$s' in " +
                    "schema '%3$s'.";
    ...
    @Inject
    public AnnotationServiceDefaultImpl(final TableBusinessRules businessRules,
                                        final JsAnnotation jsAnnotation,
    ...
    private StringBuilder getBaseKey(final DataStoreDescriptor dataStore) {
        ...
        sb.append(KeyParser.NS_SCHEMA).append(".");
        sb.append(dataStore.getDataModelDescriptor().getSchemaName()).append(".");
        sb.append(KeyParser.NS_TABLE).append(".");
    ...
    private Optional<ColumnAnnotations> getColumnAnnotations(
        ...
        if (!businessName.trim().isEmpty() || !abbrBusinessName.trim().isEmpty() ||
                !description.trim().isEmpty() || isHiddenByDefault) {
        ...
        if (!jsonExpr.isEmpty()) {
            try {
                ...
                embedded = this.jsAnnotation.getColumnAnnotations(
                        parent, column.getName(), jsonExpr);
            } catch (MalformedAnnotationException e) {
                String msg = errorWarningMessages.formatMessage(80330,
                        ERROR_MESSAGE_80330, this.getClass(),
                        column.getName(),
        ...
        Optional<ColumnAnnotations> fileBased =
                Optional.ofNullable(parent.getColumnAnnotations()
                        .get(column.getName()));

        if (result.isPresent() && fileBased.isPresent()) {
            result.get().merge(fileBased.get());
    ...
    private List<ColumnAnnotations> getColumnAnnotations(
            final TableAnnotations parent,
            final DataStoreDescriptor tableData,
            final List<Pattern> hiddenColumnPattern) {
    ...
    private Optional<TableAnnotations> getTableAnnotations(
        ...
        if (!businessName.trim().isEmpty() || !abbrBusinessName.trim().isEmpty() ||
                !description.trim().isEmpty()) {
            // create Annotation object with default annotations
            TableAnnotations ta = annotationFactory.createTableAnnotations(
                    tableData.getDataModelDescriptor()
                            .getSchemaName(),
                    tableData.getDataStoreName(),
                    businessName.trim().isEmpty()
        ...
        if (!jsonExpr.isEmpty()) {
            try {
                // retrieve from annotation in data store
                embedded = this.jsAnnotation.getTableAnnotations(
                        tableData.getDataModelDescriptor()
                                .getSchemaName(),
                        tableData.getDataStoreName(), jsonExpr);
            } catch (MalformedAnnotationException e) {
                String msg = errorWarningMessages.formatMessage(80320,
                        ERROR_MESSAGE_80320, this.getClass(),
                        tableData.getDataStoreName(),
                        tableData.getDataModelDescriptor()
                                .getSchemaName());
                errorWarningMessages.addMessage(
        ...
        try {
            ...
            String msg = errorWarningMessages.formatMessage(80310,
                    ERROR_MESSAGE_80310, this.getClass(),
                    tableData.getDataStoreName());
            errorWarningMessages.addMessage(
                    errorWarningMessages.assignSequenceNumber(), msg,
                    MESSAGE_TYPE.ERRORS);
        ...
        try {
            ...
            String msg = errorWarningMessages.formatMessage(80300,
                    ERROR_MESSAGE_80300, this.getClass(),
                    tableData.getDataStoreName());
            errorWarningMessages.addMessage(
                    errorWarningMessages.assignSequenceNumber(), msg,
                    MESSAGE_TYPE.ERRORS);
    ...
    @Override
    public Optional<TableAnnotations> getAnnotations(
            final DataStoreDescriptor tableData,
            final List<Pattern> hiddenColumnPattern) {
        //final List<Pattern> hiddenColumnPattern
        ...
        if (!tableAnnotations.isPresent()) {
            ...
            tableAnnotations = Optional.of(annotationFactory.createTableAnnotations(
                    tableData.getDataModelDescriptor()
                            .getSchemaName(),
                    tableData.getDataStoreName(),
                    null, null, null));
            ...
            // report violations
            tableAnnotations.get().isValid();
        }


        List<ColumnAnnotations> cas = getColumnAnnotations(tableAnnotations.get(),
                tableData,
                hiddenColumnPattern);
        for (ColumnAnnotations ca : cas) {
            ca.isValid();
            tableAnnotations.get().addColumnAnnotations(ca);
        }

        Optional<TableAnnotations> result;
        if (!tableAnnotations.get().isEmpty() || !cas.isEmpty()) {
