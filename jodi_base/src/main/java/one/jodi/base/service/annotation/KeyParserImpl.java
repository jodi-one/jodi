...
public class KeyParserImpl implements KeyParser {
    ...
    private final static Logger logger = LogManager.getLogger(KeyParserImpl.class);

    private final static String ERROR_MESSAGE_80600 =
            "Key '%1$s' is malformed.";

    private final static String ERROR_MESSAGE_80610 =
            "Key '%1$s' is malformed. Expected keyword '%2$s' in position %3$d.";

    private final static String ERROR_MESSAGE_80620 =
    ...
    private NameSpaceComponent parseNameSpaceComponent(final String[] elements,
                                                       final NameSpaceType type,
                                                       final int idx,
                                                       final String keyName) {
        if (!elements[idx].equalsIgnoreCase(type.getComponentName())) {
            String msg = errorWarningMessages.formatMessage(80610,
                    ERROR_MESSAGE_80610, this.getClass(),
                    keyName, type, idx);
            logger.error(msg);
            throw new IllegalArgumentException(msg);
        }
        if (elements[idx + 1].isEmpty()) {
            String msg = errorWarningMessages.formatMessage(80620,
                    ERROR_MESSAGE_80620, this.getClass(),
                    keyName, idx + 1);
            logger.error(msg);
            throw new IllegalArgumentException(msg);
        }
    ...
    public Key parseKey(final String keyString) {
        ...
        if (elements.length != 4) {
            String msg = errorWarningMessages.formatMessage(80600,
                    ERROR_MESSAGE_80600, this.getClass(),
                    keyString);
            logger.error(msg);
            throw new IllegalArgumentException(msg);
        }
        ...
        if (elements[elements.length - 1].trim().isEmpty()) {
            String msg = errorWarningMessages.formatMessage(80620,
                    ERROR_MESSAGE_80620, this.getClass(),
                    keyString, elements.length - 1);
            logger.error(msg);
            throw new IllegalArgumentException(msg);
        }
