...
public abstract class BaseCmdlineArgumentProcessor implements RunConfig {
   ...
   private static final Logger LOGGER = LogManager.getLogger(BaseCmdlineArgumentProcessor.class);

   private static final String ERROR_MESSAGE_80050 = "Usage failure %s";
   ...
   protected Options createOptions() {
      ...
      opts.addOption("c", OPTION_CONFIG, true, "configuration property file path");

      opts.addOption("pkg", OPTION_PACKAGE, true, "no defaults.");

      opts.addOption("scn", OPTION_SCENARIO, true, "no defaults.");

      Option moduleOpt = new Option(OPTION_MODULE, true, "one.jodi.bootstrap.ModuleProvider implementation");
      ...
      opts.addOption(OPTION_DEV_MODE, false, "indicates development mode semantics to the DI framework");
      ...
      opts.addOption("ps", OPTION_PACKAGE_SEQUENCE, true, "package sequence (integer)");

      opts.addOption("pw", OPTION_PASSWORD, true, "password for ODI work repository or database schema");
      opts.addOption("mpw", OPTION_WORKREP_PASSWORD, true, "password for ODI master repository");

      opts.addOption(OPTION_IDS, false, "Generate id and uid for rpd generation default value is true.");
   ...
   protected void usage(final String header, final Options opts, final int exitCode) {
      try {
         ...
         String msg = errorWarningMessages.formatMessage(80050, ERROR_MESSAGE_80050, Class.class, r.getMessage());
