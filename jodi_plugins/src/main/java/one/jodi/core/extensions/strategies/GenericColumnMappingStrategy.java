...
/**
 * <p>Implementation of the strategy object which attempts to form target mapping expressions based on source
 ...
 * </ul>
 *
 * <p> This strategy attempts to duplicate the Jodi 1.0 MagicAutoMappingComputer class' functionality.
 *
 * <p> Using this strategy also allows for automatic mapping of target columns to expressions using properties. The used format is:
 *    {@code ext.automapping.<target_column>=<expression>}
 */
public class GenericColumnMappingStrategy implements ColumnMappingStrategy {

   private static final Logger LOGGER = LogManager.getLogger(GenericColumnMappingStrategy.class);
   ...
   public String getMappingExpression(final String currentMappingExpression,
      ...
      // currentMappingExpression may be null; it assumes that the target expression is the same as
      // the source, e.g. target.column_x expression is source.column_x
      if (cmContext == null || targetColumnExecutionContext == null) {
         throw new IllegalArgumentException(
                 "Input is mandatory, no null values allowed: currentMappingExpression=" + currentMappingExpression +
                         "; cmContext=" + cmContext + "; targetColumnExecutionContext=" + targetColumnExecutionContext);
      }
      ...
      PropertyValueHolder propAutomap = cmContext.getCoreProperties()
                                                 .get(String.format(PROPERTY_COLUMN_AUTOMAP_FMT, targetColumn));
      if (propAutomap != null) {
         try {
      ...
      PropertyValueHolder propColumnMatchType = cmContext.getCoreProperties()
                                                         .get(PROPERTY_COLUMN_MATCH_TYPE);
      if (propColumnMatchType != null) {
         return getMappingOhi(MatchTypes.findMatchType(propColumnMatchType.getString()), currentMappingExpression,
                              cmContext, targetColumnExecutionContext);
      }
   ...
   private String getMappingOhi(MatchTypes matchType, String currentMappingExpression,
      ...
      if (matchType == MatchTypes.UNKNOWN) {
         RuntimeException e = new IllegalArgumentException(
                 "Column matching setting " + PROPERTY_COLUMN_MATCH_TYPE + "(" + cmContext.getCoreProperties()
                                                                                          .get(PROPERTY_COLUMN_MATCH_TYPE) +
      ...
      for (DataStoreWithAlias sourceDataStoreWithAlias : cmContext.getDataStores()) {
         if (!sourceDataStoreWithAlias.getDataStore()
                                      .getDataModel()
                                      .getModelCode()
                                      .equals("ORACLE_OHICAH_DMT") && !sourceDataStoreWithAlias.getDataStore()
                                                                                               .getDataModel()
                                                                                               .getModelCode()
                                                                                               .equals("ORACLE_OHICAH_STO")) {
            return currentMappingExpression;
         }

         String sourceAlias = sourceDataStoreWithAlias.getAlias();
         DataStore sourceDataStore = sourceDataStoreWithAlias.getDataStore();
         for (String sc : sourceDataStore.getColumns()
                                         .keySet()) {
            ...
            if (matchType == MatchTypes.MAGIC && (targetColumn.length() > 4) &&
                    sourceColumn.endsWith(targetColumn.substring(4)) && !StringUtils.containsIgnoreCase(targetColumn,
                                                                                                        cmContext.getCoreProperties()
                                                                                                                 .get(JodiConstants.W_INSERT_DT)
                                                                                                                 .getString()) &&
