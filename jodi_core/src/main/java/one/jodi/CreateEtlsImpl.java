...
public class CreateEtlsImpl implements CreateEtls, ActionRunner {
    public final static String ERROR_MESSAGE_00250 = "Parse error in file 0.xml %s";
    public final static String ERROR_MESSAGE_00260 = "Unknown error processing file 0.xml %s";
    private final static Logger logger = LogManager.getLogger(CreateEtlsImpl.class);
    private final static String ERROR_MESSAGE_00270 = "The input file/folder doesn't exist: %s";
    private final static String ERROR_MESSAGE_00280 = "The prefix configuration is required to run ETL creation";
    private final static String ERROR_MESSAGE_00290 = "The metadata directory is required to run ETL creation";
    private final static String ERROR_MESSAGE_00300 = "Generating Scenarios failed, this might happen if there are two or more packages with the same name. %s ";
    private final static String ERROR_MESSAGE_00320 = "The configuration property file is required to run ETL creation";
    private final static String ERROR_MESSAGE_00330 = "Validation for Journalizing failed.";
    ...
    @Inject
    protected CreateEtlsImpl(final TransformationService transformationService,
                             final TableServiceProvider tableService,
                             final PackageService odiPackageService,
                             final ScenarioService odiScenarioService,
                             final ProcedureService procedureService,
                             final MetadataServiceProvider metadataProvider,
                             final @InterfacePrefix String prefix,
                             final @Nullable @XmlFolderName String mappingFolder,
                             ...
                             final VariableService variablesService,
    ...
    public void regenerateProject() {
        ...
        try {
            ...
            if ((re.getCause() != null) && (re.getCause() instanceof JAXBException)) {
                msg = errorWarningMessages.formatMessage(250,
                        ERROR_MESSAGE_00250, this.getClass(), re.getCause());
                logger.error(msg, re);
            } else {
                msg = errorWarningMessages.formatMessage(260,
                        ERROR_MESSAGE_00260, this.getClass(), re);
                logger.error(msg, re);
            }
        ...
        if (journalized) {
            ...
            for (JournalizingConfiguration config : cdcDs) {
                for (DataStore ds : config.getDataStores()) {
                    i = i + 10;
                    tableService.setCDCDescriptor(ds.getDataStoreName(),
                            ds.getDataModel().getModelCode(), i);
                }
            }
        ...
        if (!file.exists()) {
            String msg = errorWarningMessages.formatMessage(270,
                    ERROR_MESSAGE_00270, this.getClass(), mappingFolder);
            ErrorReport.addErrorLine(0, msg);
            logger.error(msg);
        ...
        try {
            ...
            String message = rte.getMessage() != null ? rte.getMessage()
                    : "Unknown error occurred.";
            logger.debug(message);
            String msg = errorWarningMessages.formatMessage(300,
                    ERROR_MESSAGE_00300, this.getClass(), rte.getMessage());
            errorWarningMessages.addMessage(
                    errorWarningMessages.assignSequenceNumber(), msg,
                    MESSAGE_TYPE.ERRORS);
    ...
    protected List<String> getPackageNames(Collection<ETLPackage> etlObject) {
        List<String> packages = etlObject.stream().map(ETLPackage::getPackageName)
                .collect(Collectors
    ...
    public void validateRunConfig(RunConfig config) throws UsageException {
        ...
        if (!StringUtils.hasLength(etlConfig.getPrefix())) {
            String msg = errorWarningMessages.formatMessage(280,
                    ERROR_MESSAGE_00280, this.getClass());
            errorWarningMessages.addMessage(
                    errorWarningMessages.assignSequenceNumber(), msg,
                    MESSAGE_TYPE.ERRORS);
            logger.error(msg);
        ...
        if (!StringUtils.hasLength(config.getMetadataDirectory())) {
            String msg = errorWarningMessages.formatMessage(290,
                    ERROR_MESSAGE_00290, this.getClass());
            errorWarningMessages.addMessage(
                    errorWarningMessages.assignSequenceNumber(), msg,
                    MESSAGE_TYPE.ERRORS);
            logger.error(msg);
        ...
        if (!StringUtils.hasLength(config.getPropertyFile())) {
            String msg = errorWarningMessages.formatMessage(320,
                    ERROR_MESSAGE_00320, this.getClass());
            errorWarningMessages.addMessage(
                    errorWarningMessages.assignSequenceNumber(), msg,
                    MESSAGE_TYPE.ERRORS);
            logger.error(msg);
