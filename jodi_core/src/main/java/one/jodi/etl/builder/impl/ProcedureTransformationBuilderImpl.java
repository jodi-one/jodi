...
public class ProcedureTransformationBuilderImpl
        implements ProcedureTransformationBuilder {

    private final static Logger logger =
            LogManager.getLogger(ProcedureTransformationBuilderImpl.class);
    ...
    @Inject
    public ProcedureTransformationBuilderImpl(
    ...
    private String findDefaultTechnology(final CommandInternalImpl.Location location,
                                         final TaskInternal parent) {
    ...
    private CommandInternal build(final TaskInternal parent,
                                  final CommandType extCommand,
                                  final CommandInternalImpl.Location location,
        ...
        if (extCommand.getTechnology() == null) {
            ...
            technology = extCommand.getTechnology().value();
        }
        boolean doNotTranslate = false;
        return new CommandInternalImpl(parent, technology,
                extCommand.getLogicalSchema(),
                extCommand.getExecutionContext(),
                extCommand.getCommand(),
                location, logicalModelNames,
    ...
    private TaskInternal build(final ProcedureHeader parent, final Task extTask,
                               final Set<String> logicalModelNames) {
        TaskInternal task = new TaskInternalImpl(parent, extTask.getName(),
                extTask.isCleanup(),
                extTask.isIgnoreErrors());
    ...
    private List<String> getFolderPath(final Procedure extProcedure) {
        if (extProcedure.getFolderName() != null) {
            return Arrays.asList(extProcedure.getFolderName().split("/"));
        } else {
            return null;
        }
    }
    ...
    @Override
    public Optional<ProcedureInternal> build(final Procedure extProcedure,
                                             final String filePath) {

        final Set<String> logicalModelNames = metadataProvider.getLogicalSchemaNames();
        ProcedureInternal procedure =
                new ProcedureInternalImpl(getFolderPath(extProcedure),
                        extProcedure.getName(),
                        ...
                        extProcedure.getOptions() != null ? extProcedure.getOptions().getOption() : Collections.EMPTY_LIST,
                        filePath);

        if (extProcedure.getTasks() != null) {
            extProcedure.getTasks().getTask().stream()
                    // each task registers itself to parent
                    .forEach(t -> build(procedure, t,
                            logicalModelNames));
        }
    ...
    @Override
    public Optional<ProcedureHeader> buildHeader(final Procedure extProcedure,
                                                 final String filePath) {
        ProcedureHeader header = new ProcedureHeaderImpl(getFolderPath(extProcedure),
                extProcedure.getName(),
                filePath);
