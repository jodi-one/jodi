...
public class EnrichingBuilderImpl implements EnrichingBuilder {
   ...
   private static final Pattern PATTERN_UDF_NUMBER =
           Pattern.compile("ud(\\d+)\\z", Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE);
   ...
   @Inject
   public EnrichingBuilderImpl(final ColumnMappingContext columnMappingContext,
                               final ExecutionLocationContext executionLocationContext, final FlagsContext flagsContext,
                               final FolderNameContext folderNameContext, final JournalizingContext journalizingContext,
   ...
   @Override
   public DeleteTransformationContext createDeleteContext(final Transformation existingTransformation,
      ...
      mappings.setModel(existingTransformation.getMappings()
                                              .getModel());
      ...
      for (final Targetcolumn tc : existingTransformation.getMappings()
                                                         .getTargetColumns()) {
         final TargetcolumnImpl targetColumn = new TargetcolumnImpl();
      ...
      if (!isTemporaryTransformation(transformation.getMappings()
         ...
         final MappingsImpl mps = (MappingsImpl) transformation.getMappings();
         mps.setTargetDataStore(transformation.getMappings()
                                              .getTargetDataStore());
   ...
   private List<ExecutionLocationtypeEnum> convert(final List<ExecutionLocationType> in) {
      return in.stream()
   ...
   @Override
   public Transformation enrich(final Transformation transformation, final boolean isJournalizedData) {
      LOGGER.debug("------------------------------------------------");
      ...
      ((TransformationImpl) transformation).setTemporary(isTemporaryTransformation(transformation.getMappings()
                                                                                                 .getTargetDataStore()));
      modelCodeContext.getModelCode(transformation.getMappings());
      folderNameContext.getFolderName(transformation, isJournalizedData);
      ...
      // operations)
      int dataSetNumber = 0;
      for (final Dataset dataset : transformation.getDatasets()) {
      ...
      for (final Dataset dataset : transformation.getDatasets()) {
         for (final Source source : dataset.getSources()) {
            final SourceImpl srcAsImpl = (SourceImpl) source;
            ...
            if (journalizingContext.isJournalizedSource(srcAsImpl)) {
               journalizingContext.getJournalizingConfiguration()
                                  .stream()
                                  .filter(journalizingConfig -> journalizingConfig.getModelCode()
                                                                                  .equals(srcAsImpl.getModel()))
                                  .forEach(journalizingConfig -> srcAsImpl.setSubscribers(
                                          journalizingConfig.getSubscribers()));
            }
            for (final Lookup lookup : srcAsImpl.getLookups()) {
               final LookupImpl lkpAsImpl = (LookupImpl) lookup;
      ...
      // Now process flags
      int position = 0;
      for (final Targetcolumn tc : transformation.getMappings()
                                                 .getTargetColumns()) {
         position++;
         final TargetcolumnImpl tcAsImpl = (TargetcolumnImpl) tc;
         final TargetColumnFlags flags = flagsContext.getTargetColumnFlags(tcAsImpl);
         ...
         if (flags.isUpdateKey() == null) {
            LOGGER.error(tcAsImpl.getParent()
                                 .getTargetDataStore() + "." + tcAsImpl.getName() + " is " + flags.isUpdateKey());
         }
         ((TransformationImpl) transformation).setUseExpressions(flags.useExpression());
         if (properties.getProperty("odi12.suppressExpression") == null ||
                 properties.getProperty("odi12.suppressExpression") == null ||
                 properties.getProperty("odi12.suppressExpression")
                           .equals("false")) {
            ((TransformationImpl) transformation).setUseExpressions(true);
         } else {
            ((TransformationImpl) transformation).setUseExpressions(false);

         }
         final Set<UserDefinedFlag> userDefinedFlags = flagsContext.getUserDefinedFlags(tcAsImpl);
      ...
      for (final Targetcolumn targetColumn : transformation.getMappings()
                                                           .getTargetColumns()) {

         final List<ExecutionLocationType> executionLocations =
                 executionLocationContext.getTargetColumnExecutionLocation(targetColumn);
      ...
      transformation.getDatasets()
                    .stream()
                    .flatMap(ds -> ds.getSources()
                                     .stream())
                    .forEach(src -> {
                       ...
                       src.getLookups()
                          .forEach(etlValidator::validateJoinEnriched);
                    });
      ...
      if (properties.includeDetails()) {
         if (transformation.getComments() != null && !transformation.getComments()
                                                                    .trim()
                                                                    .equals("")) {
      ...
      if (transformation.getBeginMappingCommand() != null) {
         ((TransformationImpl) transformation).setBeginMappingCommand(
                 getMappingCmd(transformation.getBeginMappingCommand()));
      }
      if (transformation.getEndMappingCommand() != null) {
         ((TransformationImpl) transformation).setEndMappingCommand(
                 getMappingCmd(transformation.getEndMappingCommand()));
      }
   ...
   private MappingCommand getMappingCmd(final MappingCommand beginOrEndMappingCommand) {
      return new MappingCommand() {
         ...
         public String getText() {
            return beginOrEndMappingCommand.getText()
                                           .trim();
         }
   ...
   private KmType generateKMType(final KnowledgeModuleConfiguration kmc) {
      if (kmc == null) {
         final String msg = errorWarningMessages.formatMessage(3240, ERROR_MESSAGE_03240, this.getClass());
         LOGGER.error(msg);
      ...
      final KmTypeImpl type = new KmTypeImpl();
      type.setName(kmc.getName());
      for (final String k : kmc.getOptionKeys()) {
         type.addOption(k, kmc.getOptionValue(k) + "");
      }
      return type;
   }

   private Set<one.jodi.etl.internalmodel.UserDefinedFlag> convertUDFs(final Set<UserDefinedFlag> externalFlags) {
      final Set<one.jodi.etl.internalmodel.UserDefinedFlag> result = new HashSet<>(externalFlags.size());
      for (final UserDefinedFlag udf : externalFlags) {
         try {
            final Matcher regexMatcher = PATTERN_UDF_NUMBER.matcher(udf.getName());
            if (regexMatcher.matches()) {
               final int udfNumber = Integer.parseInt(regexMatcher.group(1));
               result.add(new UserDefinedFlagImpl(udfNumber, udf.getValue()));
            }
         } catch (final NumberFormatException | PatternSyntaxException e) {
            final String msg =
                    errorWarningMessages.formatMessage(8070, ERROR_MESSAGE_08070, this.getClass(), udf.getName());
   ...
   @Override
   public boolean isTemporaryTransformation(final String tableName) {
      return tempNameTableMatcher.matcher(tableName)
                                 .find();
   }
}
