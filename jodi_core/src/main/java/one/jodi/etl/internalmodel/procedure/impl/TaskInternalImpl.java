...
public class TaskInternalImpl implements TaskInternal, Validate {
    private final static Logger logger =
            LogManager.getLogger(TaskInternalImpl.class);

    private final static String EOL = System.getProperty("line.separator");

    private final static String ERROR_MESSAGE_62300 =
            "Procedure must define a non-empty task name in its definition file %1$s. " +
                    "This procedure specification will be ignored.";

    private final static String ERROR_MESSAGE_62310 =
    ...
    public TaskInternalImpl(final ProcedureHeader parent, final String name,
                            final boolean cleanup, final boolean ignoreErrors) {
        super();
        this.parent = parent;
        this.name = name;
    ...
    public boolean validate(final ErrorWarningMessageJodi errorWarningMessages) {
        ...
        if (name == null || name.isEmpty()) {
            String msg = errorWarningMessages.formatMessage(62300, ERROR_MESSAGE_62300,
                    this.getClass(),
        ...
        if (!sourceCommand.isPresent() && !targetCommand.isPresent()) {
            String msg = errorWarningMessages.formatMessage(62310, ERROR_MESSAGE_62310,
                    this.getClass(),
                    getParent().getFilePath());
            errorWarningMessages.addMessage(msg, ErrorWarningMessageJodi.MESSAGE_TYPE
                    .ERRORS);
        ...
        if (sourceCommand.isPresent()) {
            ...
            boolean srcValid = ((Validate) sourceCommand.get())
                    .validate(errorWarningMessages);
            isValid = isValid && srcValid;
        }
        if (targetCommand.isPresent()) {
            // always execute validation to capture all errors
            boolean trgValid = ((Validate) targetCommand.get())
                    .validate(errorWarningMessages);
    ...
    public String toString() {
        ...
        if (this.sourceCommand.isPresent()) {
            sb.append(this.sourceCommand.get().toString());
        }
        if (this.targetCommand.isPresent()) {
            sb.append(this.targetCommand.get().toString());
        }
        return sb.toString();
    }
