...
public class CheckTablesActionRunner implements ActionRunner {
    ...
    private final static String ERROR_MESSAGE_01040 = "The configuration property file is required to run Check Tables";
     ...
     */
    @Inject
    protected CheckTablesActionRunner(final TableServiceProvider tableService,
    ...
    public void validateRunConfig(RunConfig config) throws UsageException {
        if (!StringUtils.hasLength(config.getPropertyFile())) {
            String msg = errorWarningMessages.formatMessage(1040,
                    ERROR_MESSAGE_01040, this.getClass());
