...
public class DeleteAllPackagesActionRunner implements ActionRunner {
    ...
    private static final Logger logger = LogManager.getLogger(DeleteAllPackagesActionRunner.class);

    private static final String ERROR_MESSAGE_01053 =
            "The configuration property metadata directory is required to run the command" + "'Delete All Packages'.";
    private static final String ERROR_MESSAGE_01504 =
            "The Jodi properties file sets jodi.update to true. " + "Packages will be truncated and not deleted.";
    private static final String ERROR_MESSAGE_00250 = "Parse error in file 0.xml %s";
    private static final String ERROR_MESSAGE_00260 = "Unknown error processing file 0.xml %s";
     ...
     */
    @Inject
    protected DeleteAllPackagesActionRunner(final PackageService packageService,
    ...
    public void run(final RunConfig config) {
        ...
        try {
            ...
            if (properties.isUpdateable()) {
                String msg = errorWarningMessages.formatMessage(1054, ERROR_MESSAGE_01504, this.getClass());
    ...
    public void validateRunConfig(RunConfig config) throws UsageException {
        if (!StringUtils.hasLength(config.getMetadataDirectory())) {
            String msg = errorWarningMessages.formatMessage(1053, ERROR_MESSAGE_01053, this.getClass());
            errorWarningMessages.addMessage(errorWarningMessages.assignSequenceNumber(), msg, MESSAGE_TYPE.ERRORS);
