...
public class CreateProcedureActionRunner implements ActionRunner {
   ...
   private static final Logger logger = LogManager.getLogger(CreateProcedureActionRunner.class);
   private final boolean generateScenarioForProcedures;
   private final ProcedureService service;

   @Inject
   protected CreateProcedureActionRunner(final JodiProperties properties, final ProcedureService service) {
      this.service = service;
      if (properties.getPropertyKeys()
                    .contains(ODI12_GENERATE_SCENARIOS_FOR_PROCEDURES)) {
         this.generateScenarioForProcedures =
                 Boolean.valueOf(properties.getProperty(ODI12_GENERATE_SCENARIOS_FOR_PROCEDURES));
      } else {
