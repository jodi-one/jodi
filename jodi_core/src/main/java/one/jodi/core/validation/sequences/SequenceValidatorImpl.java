...
public class SequenceValidatorImpl implements SequenceValidator {
    ...
    private static final String ERROR_MESSAGE_4001 = "Sequence name must be unique, %s is not unique.";
    private static final String ERROR_MESSAGE_4002 = "Sequence name must be specified.";
    private static final String ERROR_MESSAGE_4003 = "Sequence schema must be specified for sequence %s.";
    private static final String ERROR_MESSAGE_4004 = "Sequence table must be specified for sequence %s.";
    private static final String ERROR_MESSAGE_4005 = "Sequence column must be specified for sequence %s.";
    private static final String ERROR_MESSAGE_4006 = "NativeSequenceName must be specified for sequence %s.";
    private static final String ERROR_MESSAGE_4008 =
            "Sequence table must exist in underlying ETLSystem for sequence %s for model %s and table %s.";
    private static final String ERROR_MESSAGE_4009 =
            "Sequence column must exist in underlying ETLSystem for sequence %s.";
    private static final String ERROR_MESSAGE_4020 = "Sequences are not valid and won't be processed.";
    private static final Logger logger = LogManager.getLogger(SequenceValidatorImpl.class);
    ...
    public boolean validate(final Sequences internalSequences) {
        ...
        boolean isUniqueName = internalSequences.getSequences()
                                                .stream()
                                                .filter(s -> !validateUniqueNames(s, uniqueNames))
                                                .count() > 0 ? false : true;
        boolean isValid = internalSequences.getSequences()
                                           .stream()
                                           .filter(s -> !validate(s, schemaNames, tableNames, columnNames))
                                           .count() > 0 ? false : true;
    ...
    private boolean validateUniqueNames(Sequence s, Set<String> uniqueNames) {
        if (uniqueNames.contains(s.getName())) {
            String message =
                    this.errorWarningMessageJodi.formatMessage(4001, ERROR_MESSAGE_4001, SequenceValidatorImpl.class,
                                                               s.getName());
            logger.error(message);
            this.errorWarningMessageJodi.addMessage(message, ErrorWarningMessageJodi.MESSAGE_TYPE.ERRORS);
            return false;
    ...
    private boolean validate(final Sequence s, final Set<String> schemaNames, final Set<String> tableNames,
        ...
        if (s.getName() == null) {
            String message =
                    this.errorWarningMessageJodi.formatMessage(4002, ERROR_MESSAGE_4002, SequenceValidatorImpl.class);
            logger.error(message);
            this.errorWarningMessageJodi.addMessage(message, ErrorWarningMessageJodi.MESSAGE_TYPE.ERRORS);
            return false;
        }
        if (s instanceof SpecificSequence) {
            ...
            if (!tableNames.contains(((SpecificSequence) s).getSchema() + "." + ((SpecificSequence) s).getTable())) {
                String message = this.errorWarningMessageJodi.formatMessage(4008, ERROR_MESSAGE_4008,
                                                                            SequenceValidatorImpl.class, s.getName(),
                                                                            ((SpecificSequence) s).getSchema(),
                                                                            ((SpecificSequence) s).getTable());
                logger.error(message);
                this.errorWarningMessageJodi.addMessage(message, ErrorWarningMessageJodi.MESSAGE_TYPE.ERRORS);
                return false;
            }
            if (!columnNames.contains(
                    (((SpecificSequence) s).getSchema() + "." + ((SpecificSequence) s).getTable() + "." +
