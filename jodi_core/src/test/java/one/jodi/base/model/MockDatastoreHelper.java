...
public class MockDatastoreHelper {
    ...
    public final static String DB_SERVER = "My Data Server";
    private final static String DB_SERVER_TECH = "Oracle";

    public static DataStore createMockDataStore(final String tableName,
                                                final String modelCode) {
        return createMockDataStore(tableName, modelCode, false);
    }

    public static DataStore createMockDataStore(final String tableName,
                                                final String modelCode, final boolean isIgnoredbyHeuristics) {
    ...
    private static DataStoreType getDataStoreType(final String tableName) {
        ...
        if (tableName.toUpperCase().endsWith("_F")) {
            dsType = DataStoreType.FACT;
        } else if (tableName.toUpperCase().endsWith("SCD2_D")) {
            dsType = DataStoreType.SLOWLY_CHANGING_DIMENSION;
        } else if (tableName.toUpperCase().endsWith("_D")) {
    ...
    public static DataStore createMockDataStore(final String tableName,
                                                final String modelCode, final String[] columns) {

        Map<String, Object> emptyFlexfieldMap = Collections.emptyMap();
        final DataModel dataModel = new DataModelImpl(modelCode, DB_SERVER,
                PHYSICAL_SERVER_NAME, DB_SERVER_TECH, "SOMESCHEMA",
    ...
    private static List<DataStoreForeignReference> createFks(
            final DataStore foreignDataStore, final Map<String, DataStore> referenced) {
        final List<DataStoreForeignReference> dsFks;
        if ((referenced == null) || (referenced.isEmpty())) {
            dsFks = Collections.emptyList();
        } else {
            dsFks = new ArrayList<DataStoreForeignReference>();
            for (Map.Entry<String, DataStore> entry : referenced.entrySet()) {
                DataStoreForeignReference fk = createMockFK(foreignDataStore,
                        entry.getKey(),
                        entry.getValue());
                dsFks.add(fk);
            }
    ...
    public static DataStoreForeignReference createMockFK(final DataStore foreign,
                                                         final String fkColumn,
                                                         final DataStore primary) {

        List<ForeignReference> fkRef = createFKRel(foreign.getDataStoreName() + "_FK",
                primary.getDataStoreName(),
                primary.getDataModel().getModelCode(),
                new String[]{fkColumn},
                false);
    ...
    public static DataStore createMockDataStore(final String tableName,
        ...
        final List<DataStoreKey> keys = new ArrayList<DataStoreKey>();
        if ((keyColumns != null) && (keyColumns.length > 0) && keyColumns[0].length() > 0) {

            final DataStoreKey.KeyType type;
            if (keyColumns[0].toLowerCase().equals("row_wid")) {
                type = DataStoreKey.KeyType.PRIMARY;
            } else if (keyColumns[0].toLowerCase().startsWith("p_")) {
                type = DataStoreKey.KeyType.PRIMARY;
            } else if (keyColumns[0].toLowerCase().startsWith("a_")) {
            ...
            keys.add(new DataStoreKey() {
                ...
                public String getName() {
                    return keyColumns[0].substring(0, 1).toUpperCase() + "_key";
                }
        ...
        final List<String> altKeyColumns = new ArrayList<String>();
        for (String column : columns) {
            if (column.toUpperCase().endsWith("_U1")) {
        ...
        final DataModel dataModel = new DataModelImpl(modelCode, DB_SERVER,
                PHYSICAL_SERVER_NAME, DB_SERVER_TECH, modelCode,
                emptyFlexfieldMap, "UNKNOWN", false, "ORCL", 1521);
    ...
    public static DataStore createMockDataStore(final String tableName,
                                                final String modelCode, final boolean isIgnoredbyHeuristics, final String[] keyColumns) {
        ...
        if ((keyColumns != null) && (keyColumns.length > 0) && keyColumns[0].length() > 0) {
            ...
            final DataStoreKey.KeyType type;
            if (keyColumns[0].toLowerCase().startsWith("p")) {
                type = DataStoreKey.KeyType.PRIMARY;
            } else if (keyColumns[0].toLowerCase().startsWith("a")) {
                type = DataStoreKey.KeyType.ALTERNATE;
            } else if (keyColumns[0].toLowerCase().startsWith("u")) {
                type = DataStoreKey.KeyType.ALTERNATE;
            } else {
                type = DataStoreKey.KeyType.INDEX;
            }
            ...
            keys.add(new DataStoreKey() {
                ...
                public String getName() {
                    ...
                    return keyColumns[0].substring(0, 1).toUpperCase() + ((keyColumns[0].toLowerCase().startsWith("u")) ? "_U1" : "_key");
                }
    ...
    public static Map<String, DataStoreColumn> createDataStoreColumns(
            final DataStore parent, final String[] columnNames) {

        Map<String, DataStoreColumn> columns = new HashMap<String, DataStoreColumn>();
        int position = 1;
    ...
    public static List<ForeignReference> createFKRel(final String relationshipKeyName,
        ...
        final List<ForeignReference> fkRefs = new ArrayList<ForeignReference>();

        final List<ForeignReference.RefColumns> refColumns = new ArrayList<ForeignReference.RefColumns>();
    ...
    public static Map<String, DataStoreDescriptor> createMockDSDescriptor(final String dataStoreName,
                                                                          final String modelCode, final boolean isTemporaryTable) {
        return createMockDSDescriptor(dataStoreName, modelCode, isTemporaryTable, new String[]{}, new String[]{},
                null, null, new String[]{});
    }

    public static Map<String, DataStoreDescriptor> createMockDSDescriptor(final String dataStoreName,
                                                                          final String modelCode, final boolean isTemporaryTable, final String[] columns) {
        return createMockDSDescriptor(dataStoreName, modelCode, isTemporaryTable, columns, new String[]{},
                null, null, new String[]{});
    }
    ...
    public static Map<String, DataStoreDescriptor> createMockDSDescriptor(final String dataStoreName,
                                                                          final String modelCode, final boolean isTemporaryTable, final String[] columns,
                                                                          final String[] keyColumns) {
        return createMockDSDescriptor(dataStoreName, modelCode, isTemporaryTable, columns, keyColumns,
                null, null, new String[]{});
    }
    ...
    public static Map<String, DataStoreDescriptor> createMockDSDescriptor(final String dataStoreName,
                                                                          final String modelCode, final boolean isTemporaryTable, final String[] columns,
                                                                          final String[] keyColumns,
                                                                          final String primaryKeyDataStoreName, final String primaryKeyModelCode,
                                                                          final String[] refKeyColumns) {

        final String relationshipKeyName = dataStoreName + "_" + primaryKeyDataStoreName + "_FK";

        Map<String, DataStoreDescriptor> dsDescMap = new HashMap<String, DataStoreDescriptor>();
